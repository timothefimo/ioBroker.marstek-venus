...
class MarstekVenus extends utils.Adapter {
    ...
    async pollBattery() {
        const r = await this.sendCommand('Bat.GetStatus', { id: null });
        if (!r) return;
    ...
    async pollES() {
        ...
        const modeRes = await this.sendCommand('ES.GetMode', { id: null });
        if (modeRes) {
        ...
        // Get ES status
        const r = await this.sendCommand('ES.GetStatus', { id: null });
        if (!r) return;
    ...
    async pollEM() {
        const r = await this.sendCommand('EM.GetStatus', { id: null });
        if (!r) return;
    ...
    async pollPV() {
        const r = await this.sendCommand('PV.GetStatus', { id: null });
        if (!r) return;
    ...
    async setMode(mode) {
        ...
        const r = await this.sendCommand('ES.SetMode', { id: null, mode: modeNum });
        if (r && r.set_result) {
    ...
    async setDOD(dod) {
        ...
        const r = await this.sendCommand('DOD.Set', { id: null, dod });
        if (r && r.set_result) {
    ...
    async setManualSchedule(schedule) {
        // schedule: array of { start, end, power, mode } objects
        if (!Array.isArray(schedule) || schedule.length === 0) {
            this.log.warn('setManualSchedule: expects a non-empty array of schedule entries');
            return;
        }
        const r = await this.sendCommand('ES.SetManual', { id: null, schedule });
        if (r && r.set_result) {
